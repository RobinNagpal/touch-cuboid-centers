...
from cuboid_cell.world import build_world, random_cuboids
...
def robot_description(share: Path) -> str:
    ...
    return xacro.process_file(
        str(share / "urdf" / "cell.urdf.xacro"),
        mappings={"controllers_file": str(share / "config" / "controllers.yaml")},
    ).toxml()


def write_world(share: Path, count: int, seed: int) -> str:
    template = (share / "worlds" / "cell.sdf").read_text()
    sdf = build_world(template, random_cuboids(count, seed))
    path = Path(tempfile.mkdtemp(prefix="cuboid_cell_")) / "cell.sdf"
    path.write_text(sdf)
    return str(path)
    ...
    return [
        ...
        *(
        ...
        Node(
            ...
            executable="create",
            output="screen",
            arguments=["-topic", "robot_description", "-name", "cuboid_cell"],
        ),
        Node(
            ...
            output="screen",
            parameters=[
                {"config_file": str(share / "config" / "gz_bridge.yaml")},
                {"use_sim_time": True},
            ],
        ),
        ...
        *_chain_spawners("joint_state_broadcaster", "arm_controller", "gripper_controller"),
        Node(
            ...
            arguments=["-d", str(share / "config" / "view.rviz")],
    ...
    return LaunchDescription(
        [
            ...
            SetEnvironmentVariable(
                ...
                str(Path(get_package_share_directory(PACKAGE)) / "config" / "fastdds.xml"),
            ),
